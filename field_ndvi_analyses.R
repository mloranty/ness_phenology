...
setwd('L:/data_repo/field_data/viperData/sensor/decagon/ndvi/')
...
#######################################################################################
#analyze field observations of phenology

p <- read.csv('C:/Users/mloranty/Documents/GitHub/ness_phenology/Cherskii_phenology_summary.csv')
...
for(i in 1:nrow(p.ts))
{
	o <- as.POSIXct(paste(p.ts$year[i],"-01-01", sep=""), tz = "GMT")
	
	p.ts$sos.s[i] <- julian(strptime(paste(p$year[i],sprintf("%02d",p$m.sos.s[i]),sprintf("%02d",p$d.sos.s[i]),sep=""),"%Y%m%d",tz="GMT"),origin=o)
	p.ts$sos.e[i] <- julian(strptime(paste(p$year[i],sprintf("%02d",p$m.sos.e[i]),sprintf("%02d",p$d.sos.e[i]),sep=""),"%Y%m%d",tz="GMT"),origin=o)
	p.ts$eos.s[i] <- julian(strptime(paste(p$year[i],sprintf("%02d",p$m.eos.s[i]),sprintf("%02d",p$d.eos.s[i]),sep=""),"%Y%m%d",tz="GMT"),origin=o)
	p.ts$eos.e[i] <- julian(strptime(paste(p$year[i],sprintf("%02d",p$m.eos.e[i]),sprintf("%02d",p$d.eos.e[i]),sep=""),"%Y%m%d",tz="GMT"),origin=o)
}

p.ts$sos <- rowMeans(p.ts[,11:12])
